...
SELECT melon_type, common_name, price AS price_usd, price * 0.73 AS price_euro FROM melons;
...
SELECT COUNT (*) FROM orders WHERE shipto_state IN ('California', 'CA');
...
SELECT melons.common_name, melons.melon_type, order_items.quantity, order_items.unit_price, order_items.total_price FROM order_items JOIN melons ON (melons.id=order_items.melon_id) WHERE order_items.order_id = 2725;
...
SELECT salespeople.first_name, salespeople.last_name, SUM(orders.order_total), SUM(orders.order_total) * .15 AS commission FROM salespeople LEFT JOIN orders ON (salespeople.id = orders.salesperson_id) GROUP BY salespeople.id; 